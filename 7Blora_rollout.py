# 评估“微调后的 7B + LoRA”在环境里的效果（few-shot + history 不变） colab
...
sys.path.append("/content/drive/MyDrive/BEAR_LLM/code")

os.environ["BASE_MODEL"] = "Qwen/Qwen2.5-7B-Instruct"
os.environ["FINETUNE_ADAPTER"] = "/content/drive/MyDrive/BEAR_LLM/ft_out_offline_ppo_7b_lora"

# 避免走 flash-attn 路径保持 SDPA
os.environ.setdefault("ATTN_IMPL", "sdpa")

SAVE_PATH  = os.getenv(
    "SAVE_PATH",
    "/content/drive/MyDrive/BEAR_LLM/outputs/mini_rollout_fewshot_7B_finetuned.json"
)
...
from llm_agent_colabfinetune import call_llm, parse_actions
...
def main():
    ...
    try:
        ...
        dprint(f"[fewshot] loaded {len(EX_DATASET)} examples from {FEWSHOT_JSON}")
    ...
    for step in range(MAX_STEPS):
        ...
        obs = obs_next
        if done:
            dprint(f"[done] early stop at step {step}")
            break
